...
import services.interfaces.IFastOrderService;
import services.interfaces.INormalOrderService;
import services.interfaces.IPayService;
...
public class Client implements Runnable {
    ...
    private INormalOrderService normalOrderService;

    public Client (Store store){
        this.fastOrderService = store;
        this.normalOrderService = store;
        this.payService = store;
    }
    // -----------------------------------------------------
    ...
    InsufficientBalanceException, UnknownAccountException{
        ...
        Order order = fastOrderService.oneShotOrder(this,item,qty,address,account);
    ...
    UnknownItemException, InvalidCartException{
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            cart = normalOrderService.addItemToCart(cart,this,items[i],qties[i]);
        }
        Order order = payService.pay(cart,address,account);
        System.out.println(order);
    }
}
